...
本文主要整合了对上述文档的翻译、其他的相关资料以及作者自己的编程经验，力求总结出一份通用的编码规范。
...
推荐：将带有分支的代码（`if`, `else`, `switch`, `while` 等等）和上一个分支的结束括号（`)`, `}`），以及自己分支的开始括号（`(`, `{`）放在同一行，来保证代码的紧凑可读。查看苹果SDK的API文档，也可以看到类似的做法。
...
如果构造代码不写在一行内，构造元素需要使用**两个空格**来进行缩进，右括号 `]` 或者 `}`写在新的一行：
...
```objective-c
// 使用 FS前缀，表示业务模块的组件
@interface FSWebViewController : UIViewController
...
@protocol FSVersionUpdateProtocol <NSObject>
...
@end
```
...
#pragma mark - FSVideoPlayerControlDelegate
